...
if($result=$this->connection->query($quest)){
	...
	$dop=array(id_ap=>$row['id_ap'],name=>$row['name'],adress=>$row['adress'],phone=>$row['phone'],photo=>$row['photo']);
    $spisok[]=$dop;
    }
    $result->close();
  }
...
$quest = "INSERT INTO `BD_Aptek`.`apteki` (`id_ap`, `name`, `adress`, `phone`, `photo`) VALUES (NULL, '".$mass2[name]."', '".$mass2[adress]."', '".$mass2[phone]."', '".$mass2[photo]."')";
if($result=$this->connection->query($quest))
echo $quest ;
else echo $quest ;
 }
...
$dop=array(name=>$row['name'],adress=>$row['adress'],
			phone=>$row['phone'],photo=>$row['photo']);
	return($dop);	      
 }
	}
